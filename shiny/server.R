...
server <- function(input, output, session) {
  ...
  # method = "near" is REQUIRED for categorical/classified rasters (e.g. the
  # 6-class SUHII map): bilinear (the default) interpolates between class
  # integers at every class boundary, producing non-integer values that
  # colorFactor() can't match -> NA -> transparent "holes" along every
  # transition between classes.
  to_4326 <- function(r, method = "bilinear") terra::project(r, "EPSG:4326", method = method)
  ...
  output$map_lst <- renderLeaflet({
    ...
    rng  <- range(vals)                         # full min/max: every pixel is guaranteed in-domain
    pal  <- colorNumeric("Spectral", domain = rng,
                          reverse = TRUE, na.color = "transparent")
    leaflet() |>
      addProviderTiles("CartoDB.Positron", group = "Light") |>
      addProviderTiles("Esri.WorldImagery", group = "Satellite") |>
      addRasterImage(r, colors = pal, opacity = 0.8, group = "Mean LST") |>
      addLegend("bottomright", pal = pal, values = rng,
                title = "LST (°C)", labFormat = labelFormat(suffix = "°C", digits = 0)) |>
  ...
  output$map_classified <- renderLeaflet({
    ...
    r   <- to_4326(terra::rast(res$path_classified), method = "near")
    pal <- colorFactor(CLASS_COLOURS, domain = 1:6, na.color = "transparent")
    leaflet() |>
      addProviderTiles("CartoDB.Positron", group = "Light") |>
      addProviderTiles("Esri.WorldImagery", group = "Satellite") |>
      addRasterImage(r, colors = pal, opacity = 0.8, method = "ngb", group = "SUHII classes") |>
  ...
  output$map_priority <- renderLeaflet({
    ...
    leaflet() |>
      addProviderTiles("CartoDB.Positron", group = "Light") |>
      addProviderTiles("Esri.WorldImagery", group = "Satellite") |>
      addRasterImage(r, colors = pal, opacity = 0.85, group = "Priority index") |>
      addLegend("bottomright", pal = pal, values = c(0,1),
                title = "Priority index",
                labFormat = labelFormat(digits = 2)) |>
      addLayersControl(baseGroups = c("Light","Satellite"),
                       overlayGroups = "Priority index",
                       options = layersControlOptions(collapsed = FALSE),
                       position = "topleft")
  })
  ...
  output$lst_warning <- renderUI({
    ...
    tags$div(
      ...
      tags$a("Learn more", href = "#",
             onclick = "Shiny.setInputValue('learn_more_click', Math.random()); return false;"),
      tags$button(type = "button", class = "btn-close", `data-bs-dismiss` = "alert")
    )
  })

  # "Learn more" jumps to the About tab (main_tabs id set in ui.R)
  observeEvent(input$learn_more_click, {
    bslib::nav_select("main_tabs", selected = "about", session = session)
  })
