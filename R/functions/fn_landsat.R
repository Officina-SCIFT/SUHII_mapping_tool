...
download_landsat <- function(aoi_bb, city_slug, season, max_cloud, dirs) {
  ...
  .stac_query <- function(cloud_limit) {
    tryCatch({
      ...
      # Retrieve up to 200 scenes and filter by cloud cover in R.
      ...
      # Filter client-side: cloud cover only (remove T1 filter — category field
      # may not be present or have different values on some scenes)
      features <- items$features
      if (length(features) == 0) return(items)
      ...
      cli::cli_abort("STAC query failed: {e$message}")
    })
  }
