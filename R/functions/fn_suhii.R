  ...
  tryCatch({
    leaflet::leaflet() |> leaflet::addTiles() |>
      leaflet::addRasterImage(terra::project(anomaly_map, "EPSG:4326"),
                              colors = pal_a, opacity = 0.8, maxBytes = Inf) |>
    ...
    cli::cli_alert_warning("Leaflet preview (thermal anomaly) not generated: {e$message}")
  })
  ...
  tryCatch({
    leaflet::leaflet() |> leaflet::addTiles() |>
      leaflet::addRasterImage(terra::project(suhi_map, "EPSG:4326"),
                              colors = pal_s, opacity = 0.8, maxBytes = Inf) |>
      leaflet::addLegend("bottomright", pal = pal_s, values = c(0,1),
                         title = "SUHII index") |> print()
  }, error = function(e) {
    cli::cli_alert_warning("Leaflet preview (SUHII) not generated: {e$message}")
  })
  ...
  tryCatch({
    leaflet::leaflet() |> leaflet::addTiles() |>
      leaflet::addRasterImage(terra::project(distance_raster, "EPSG:4326"),
                              colors = pal_d, opacity = 0.8, maxBytes = Inf) |>
      leaflet::addLegend("bottomright", pal = pal_d, values = c(300L, max_d),
                         title = "Distance from green (m)") |> print()
  }, error = function(e) {
    cli::cli_alert_warning("Leaflet preview (green distance) not generated: {e$message}")
  })
