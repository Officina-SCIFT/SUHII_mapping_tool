...
compute_green_distance <- function(aoi_utm, lst_mean, city_slug,
  ...
  green_vect <- terra::vect(file.path(dirs$input, "rural_areas.shp")) |>
                  terra::aggregate() |> terra::project(lst_mean)
  ...
  green_rast      <- terra::rasterize(green_vect, lst_mean, field = 1, background = NA)
  distance_raster <- terra::distance(green_rast) |> terra::mask(aoi_utm)
