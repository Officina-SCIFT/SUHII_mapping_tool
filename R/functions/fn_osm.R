  ...
  if (file.exists(bound_path) && file.size(bound_path) > 100) {
    ...
    for (lvl in c("8", "7", "6")) {
      ...
      if (!is.null(gdf) && nrow(gdf) > 0) {
        # Sanity check: warn (non-blocking) if the matched boundary is
        # implausibly large for a municipality — most often means a
        # province/region-level entity slipped through (e.g. name-substring
        # collisions like "Cuneo" vs "Provincia di Cuneo"), even with the
        # anchored exact-name match above.
        area_km2 <- tryCatch(
          sum(terra::expanse(terra::vect(gdf), unit = "km", transform = TRUE)),
          error = function(e) NA_real_)
        if (!is.na(area_km2) && area_km2 > 1500) {
          cli::cli_alert_warning(
            "Boundary matched at admin_level {lvl} is {round(area_km2)} km\u00b2 \u2014 unusually large for a municipality. This may be a province/region instead of the city. Verify boundaries.shp before trusting downstream results."
          )
        }
        cli::cli_alert_success("Boundary found at admin_level {lvl} ({nrow(gdf)} polygon(s), {round(area_km2,1)} km\u00b2)")
        aoi_sf <- gdf
        break
      }
    }
